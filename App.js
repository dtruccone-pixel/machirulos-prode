...
import { PARTIDOS, GRUPOS_KEYS, FLAG, calcPts, PARTIDOS_R32 } from "./datos";
...
export default function App(){
  ...
  if(scr==="inicio")return(
    <div style={S.root}>
      ...
      <div style={{...S.wrap,display:"flex",flexDirection:"column",gap:10,marginTop:10}}>
        ...
        <div style={{
          background:"linear-gradient(135deg,#0a1a0a,#0d0d1a)",
        ...
        {/* SECCIÓN 2 — 16avos de Final */}
        <div style={{
          background:"linear-gradient(135deg,#1a0800,#0d0d00)",
          ...
          <button style={S.btn("linear-gradient(90deg,#e67e22,#b05a0f)","#fff","none",{width:"100%",padding:"13px"})}
            onClick={()=>setScr("r32")}>⚔️ Ver 16avos y pronósticos</button>
        </div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              ...
              <button style={S.btn("transparent","#e67e22","1px solid #e67e22")}
                onClick={()=>setScr("r32")}>⚔️ 16avos</button>
  ...
  if(scr==="r32"){
    ...
    const totalPtsR32 = PARTIDOS_R32.reduce((a,p)=>{
      const res=ress[p.id], pr=mp[p.id];
      if(!pr||!res||res.l===""||res.v==="")return a;
      return a+(calcPts(pr,res)??0);
    },0);
    ...
    // Tabla R32
    const tablaR32 = jugadoresData ? calcTablaFechaCustom(jds,prons,ress,PARTIDOS_R32) : [];
    function calcTablaFechaCustom(jds2,prons2,ress2,partidos2){
      return jds2.map(j=>{
        let pts=0,ex=0,gn=0,fa=0;
        partidos2.forEach(p=>{
      ...
      <div style={S.root}>
        ...
        <div style={S.wrap}>
          ...
          <div style={{
            ...
            <p style={{...S.sTitle,color:"#e67e22",margin:"0 0 4px"}}>⚔️ Round of 32</p>
            <p style={{fontSize:18,fontWeight:900,color:"#fff",margin:0}}>16avos de Final</p>
            <p style={{fontSize:11,color:MUTED,margin:"4px 0 0"}}>28 Jun – 4 Jul · 16 partidos</p>
          </div>

          <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
            <button style={S.btn("transparent",MUTED,"1px solid #222")}
              onClick={()=>setScr("inicio")}>← Volver</button>
            {jugador&&<button style={S.btn("transparent","#e67e22","1px solid #e67e22")}
              onClick={()=>setScr("inicio")}>
              {jugador.nombre}: {totalPtsR32} pts R32
            </button>}
            <button style={S.btn("#1a1a2a","#888","1px solid #333")}
              onClick={cargarTodo}>🔄 Actualizar</button>
          </div>

          {/* Si no está logueado, mostrar botón para elegir jugador */}
          {!jugador&&(
            <div style={{...S.card,textAlign:"center",padding:24}}>
              <p style={{color:MUTED,margin:"0 0 12px"}}>Iniciá sesión para cargar tus pronósticos de 16avos</p>
          ...
          {PARTIDOS_R32.map(p=>{
            ...
            const localKnown=!p.local.startsWith("1ro")&&!p.local.startsWith("2do")&&!p.local.startsWith("3ro");
            const visKnown=!p.vis.startsWith("1ro")&&!p.vis.startsWith("2do")&&!p.vis.startsWith("3ro");
            ...
            return(
              <div key={p.id} style={{background:bg,borderRadius:12,marginBottom:10,
                ...
                {/* Meta */}
                <div style={{fontSize:10,color:MUTED,marginBottom:10,letterSpacing:1}}>
                  ⚔️ {p.fecha} {p.hora} · 📍 {p.sede}
                  {hRes&&<span style={{marginLeft:8,color:"#2a2a4a"}}>· JUGADO</span>}
                </div>
                {/* Equipos */}
                <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                  <div style={{flex:"1 1 110px",display:"flex",alignItems:"center",gap:6}}>
                    {localKnown&&<span style={{fontSize:20}}>{FLAG[p.local]||"🏳️"}</span>}
                    <span style={{fontSize:14,fontWeight:localKnown?700:400,
                      color:localKnown?TEXT:MUTED}}>{p.local}</span>
                  </div>
                  {/* Inputs — solo si ambos equipos son conocidos y está logueado */}
                  {jugador&&localKnown&&visKnown?(
                    <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
                  ...
                  <div style={{flex:"1 1 110px",display:"flex",alignItems:"center",
                    justifyContent:"flex-end",gap:6}}>
                    <span style={{fontSize:14,fontWeight:visKnown?700:400,
                      color:visKnown?TEXT:MUTED,textAlign:"right"}}>{p.vis}</span>
                    {visKnown&&<span style={{fontSize:20}}>{FLAG[p.vis]||"🏳️"}</span>}
                  </div>
                </div>
                {/* Resultado */}
                {hRes&&jugador&&(
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
                    ...
                    <div>
                      <div style={{color:"#3a3a6a",fontSize:11,textTransform:"uppercase"}}>Real</div>
                      <div style={{fontWeight:900,fontSize:18}}>{res.l} – {res.v}</div>
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <span style={{fontSize:22}}>{pts===3?"🎯":pts===2?"✅":pts===0?"❌":"⏳"}</span>
                      <div style={{padding:"3px 10px",borderRadius:8,border:`2px solid ${ptC}`,
                        fontWeight:900,fontSize:18,color:ptC}}>{pts} pts</div>
                    </div>
                  </div>
                )}
              </div>
          ...
          {tablaR32.some(j=>j.pts>0)&&(
            <div style={{...S.card,marginTop:8}}>
              ...
              {tablaR32.map((item,i)=><TablaFila key={item.jugador.nombre} item={item} pos={i} maxPts={tablaR32[0]?.pts??0}/>)}
            </div>
          )}
    ...
    if(juegoActivo&&lider) return(
      ...
      >
        ...
        <div style={{
          ...
          {/* ── BARRABRAVA LANZADOR — imagen real ── */}
          {!derribado&&(
            <div style={{
              display:"flex",flexDirection:"column",alignItems:"center",
              position:"relative",
            }}>
              {/* Imagen real del barrabrava */}
              <div style={{
                position:"relative",
                width:190,height:260,
                animation:tambaleo?"lanzar .35s ease forwards":"barraIdle 2s ease-in-out infinite",
                transformOrigin:"center bottom",
              }}>
              {/* La imagen del barrabrava */}
                <img src="/barra.png" alt="barra"
                  style={{
                    width:190,height:260,
                    objectFit:"contain",
                    objectPosition:"center",
                    filter:"drop-shadow(4px 8px 10px rgba(0,0,0,.8))",
                    display:"block",
                  }}
                />
              </div>
              ...
              {/* Polvo */}
              {tambaleo&&[0,1,2].map(i=>(
                <div key={i} style={{
                  position:"absolute",bottom:-4,
                  left:`${25+i*20}%`,
                  width:8,height:8,borderRadius:"50%",
                  background:"#555",
                  animation:`humo .5s ease ${i*.08}s forwards`,
                }}/>
              ))}
            </div>
          )}
          ...
          {/* RESULTADOS R32 */}
          <div style={S.card}>
            <p style={{...S.sTitle,color:"#e67e22"}}>⚔️ Resultados 16avos de Final</p>
            {PARTIDOS_R32.map(p=>{
              const rl=resLocal[p.id]||{l:ress[p.id]?.l||"",v:ress[p.id]?.v||""};
              const guardado=guardadoOk[p.id];
              const cargandoP=guardando[p.id];
              const listoParaGuardar=String(rl.l).trim()!==""&&String(rl.v).trim()!=="";
              const sinCambios=rl.l===(ress[p.id]?.l||"")&&rl.v===(ress[p.id]?.v||"");
              const ok=ress[p.id]?.l!==""&&ress[p.id]?.v!=="";
              return(
                <div key={p.id} style={{padding:"10px 0",borderBottom:`1px solid ${BDR}`}}>
                  <div style={{fontSize:10,color:MUTED,marginBottom:6}}>{p.fecha} {p.hora}</div>
                  <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                    <div style={{flex:"1 1 90px",fontSize:13,display:"flex",alignItems:"center",gap:4}}>
                      <span style={{fontSize:16}}>{FLAG[p.local]||"⚽"}</span>{p.local}
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:5,flexShrink:0}}>
                      <input style={{...S.inpGol(false),fontSize:18}} type="text"
                        inputMode="numeric" maxLength={2} value={rl.l} placeholder="-"
                        onChange={e=>setResLocal(prev=>({...prev,[p.id]:{...(prev[p.id]||{l:"",v:""}),l:e.target.value.replace(/[^0-9]/g,"")}}))}
                        onKeyDown={e=>{if(e.key==="Enter")guardarPartido(p.id);}}/>
                      <span style={{color:MUTED}}>–</span>
                      <input style={{...S.inpGol(false),fontSize:18}} type="text"
                        inputMode="numeric" maxLength={2} value={rl.v} placeholder="-"
                        onChange={e=>setResLocal(prev=>({...prev,[p.id]:{...(prev[p.id]||{l:"",v:""}),v:e.target.value.replace(/[^0-9]/g,"")}}))}
                        onKeyDown={e=>{if(e.key==="Enter")guardarPartido(p.id);}}/>
                    </div>
                    <div style={{flex:"1 1 90px",fontSize:13,display:"flex",
                      alignItems:"center",justifyContent:"flex-end",gap:4}}>
                      {p.vis}<span style={{fontSize:16}}>{FLAG[p.vis]||"⚽"}</span>
                    </div>
                    <button disabled={!listoParaGuardar||cargandoP}
                      style={{
                        padding:"6px 10px",borderRadius:7,cursor:listoParaGuardar&&!cargandoP?"pointer":"default",

      ...
      <div style={S.root}>
        ...
        <div style={S.wrap}>
          ...
          {PARTIDOS_R32.map(p=>{
            ...
            const lK=!p.local.startsWith("1ro")&&!p.local.startsWith("2do")&&!p.local.startsWith("3ro");
            const vK=!p.vis.startsWith("1ro")&&!p.vis.startsWith("2do")&&!p.vis.startsWith("3ro");
            return(
              <div key={p.id} style={{background:bg,borderRadius:12,marginBottom:10,
                padding:14,border:`2px solid ${bc}`}}>
                <div style={{fontSize:10,color:MUTED,marginBottom:10,letterSpacing:1}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                  <div style={{flex:"1 1 100px",display:"flex",alignItems:"center",gap:6}}>
                    {lK&&<span style={{fontSize:20}}>{FLAG[p.local]||"🏳️"}</span>}
                    <span style={{fontSize:14,fontWeight:lK?700:400,color:lK?TEXT:MUTED}}>{p.local}</span>
                  </div>
                  {jugador&&lK&&vK?(
                    <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
                  ...
                  <div style={{flex:"1 1 100px",display:"flex",alignItems:"center",
                    justifyContent:"flex-end",gap:6}}>
                    <span style={{fontSize:14,fontWeight:vK?700:400,color:vK?TEXT:MUTED,textAlign:"right"}}>{p.vis}</span>
                    {vK&&<span style={{fontSize:20}}>{FLAG[p.vis]||"🏳️"}</span>}
                  </div>
                </div>
                {hRes&&jugador&&(
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
    ...
    return(
      <div style={S.root}>
        ...
        <div style={S.wrap}>
          ...
          {/* RESULTADOS R32 */}
          <div style={S.card}>
            <p style={{...S.sTitle,color:"#e67e22"}}>Resultados 16avos</p>
            {PARTIDOS_R32.map(p=>{
              ...
              return(
                <div key={p.id} style={{padding:"10px 0",borderBottom:`1px solid ${BDR}`}}>
                  <div style={{fontSize:10,color:MUTED,marginBottom:4}}>{p.fecha} {p.hora}</div>
                  <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                    <div style={{flex:"1 1 80px",fontSize:12}}>{FLAG[p.local]||""} {p.local}</div>
                    ...
                    <div style={{flex:"1 1 80px",fontSize:12,textAlign:"right"}}>{p.vis} {FLAG[p.vis]||""}</div>

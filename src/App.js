...
export default function App(){
  ...
  if(scr==="tabla"){
    ...
    function tirarPiedra(e){
      ...
      // El muñequito siempre lanza desde abajo
      const ox=rect.left+rect.width*0.5;
      const oy=rect.top+rect.height*0.85;
      setPiedras(prev=>[...prev,{id,x:ox,y:oy,tx:cx,ty:cy}]);
      ...
      >
        <style>{`
          ...
          @keyframes derribadoAnim{
            ...
            40%{transform:rotate(35deg) translateY(10px)}
            70%{transform:rotate(75deg) translateY(30px)}
            100%{transform:rotate(95deg) translateY(60px);opacity:0}
          }
          @keyframes muneco{
          ...
          @keyframes brazo{
            ...
            30%{transform:rotate(-60deg) translateY(-8px)}
        ...
        <div style={{position:"absolute",top:16,left:0,right:0,textAlign:"center",zIndex:10}}>
          <p style={{color:GOLD,fontSize:12,letterSpacing:3,textTransform:"uppercase",margin:0}}>
            🪨 ¡Sacalo del primer puesto!
          </p>
          {!derribado&&<p style={{color:"#444",fontSize:11,margin:"3px 0 0"}}>
            Tocá para tirar piedras ({5-golpes} restantes)
          </p>}
        </div>
        ...
        <div style={{
          ...
          justifyContent:"center",width:"100%",paddingTop:80,paddingBottom:120,
          position:"relative",
        }}>
          {/* FOTO DEL LÍDER — objetivo */}
          <div style={{
            ...
            {lider.fotoUrl
              ...
              :<div style={{
                  width:140,height:140,borderRadius:"50%",background:"#111",
                  border:`4px solid ${GOLD}`,display:"flex",alignItems:"center",
                  justifyContent:"center",fontSize:64,
                  boxShadow:`0 0 30px ${GOLD}55`,
                }}>👤</div>
            }
            {/* Estrellas al ser golpeado */}
            {tambaleo&&<div style={{
              position:"absolute",top:-10,right:-10,
              fontSize:20,animation:"boom .4s ease",
            }}>💫</div>}
          </div>
          ...
          {!derribado&&<div style={{
            ...
            letterSpacing:3,color:TEXT,marginBottom:32,
          }}>{lider.nombre}</div>}

          {/* Mensaje derribado */}
          {derribado&&(
            <div style={{textAlign:"center",marginTop:16}}>
              <div style={{fontSize:52,animation:"boom .5s ease"}}>💥⭐💥</div>
              <p style={{color:GOLD,fontWeight:900,fontSize:22,
                textTransform:"uppercase",letterSpacing:3,margin:"10px 0 4px"}}>
                ¡Derribado!
              </p>
          ...
          {/* MUÑEQUITO LANZADOR (hecho con SVG) con la foto del jugador logueado */}
          {!derribado&&(
            <div style={{
              position:"absolute",bottom:0,left:"50%",
              transform:"translateX(-50%)",
              display:"flex",flexDirection:"column",alignItems:"center",
              animation:`muneco 1s ease-in-out infinite`,
            }}>
              {/* Cabeza con foto si disponible */}
              <div style={{
                width:44,height:44,borderRadius:"50%",overflow:"hidden",
                border:`3px solid ${BLUE}`,marginBottom:2,flexShrink:0,
                background:"#111",
              }}>
                {jugador?.fotoUrl
              ...
              {/* SVG cuerpo + brazo lanzador */}
              <svg width="60" height="70" viewBox="0 0 60 70" style={{overflow:"visible"}}>
                {/* Cuerpo */}
                <rect x="20" y="0" width="20" height="28" rx="6" fill={BLUE}/>
                {/* Brazo lanzador (derecho) */}
                <g style={{transformOrigin:"20px 8px",animation:`brazo .8s ease-in-out infinite`}}>
                  <rect x="0" y="4" width="22" height="8" rx="4" fill={BLUE}/>
                  {/* Mano con piedra */}
                  <circle cx="-2" cy="8" r="7" fill="#888"/>
                  <text x="-8" y="13" fontSize="12">🪨</text>
                </g>
                {/* Brazo izquierdo */}
                <rect x="40" y="4" width="14" height="8" rx="4" fill={BLUE}/>
                {/* Piernas */}
                <rect x="20" y="26" width="8" height="24" rx="4" fill="#444"/>
                <rect x="32" y="26" width="8" height="24" rx="4" fill="#444"/>
                {/* Pies */}
                <ellipse cx="24" cy="50" rx="10" ry="5" fill="#222"/>
                <ellipse cx="36" cy="50" rx="10" ry="5" fill="#222"/>
              </svg>
        ...
        {piedras.map(p=>(
          <div key={p.id} style={{
            ...
            animation:"piedraVuela .65s cubic-bezier(.2,.8,.4,1) forwards",
            zIndex:50,
          }}>🪨</div>
        ))}
        ...
        {/* Indicador de golpes */}
        <div style={{
          position:"absolute",bottom:80,display:"flex",gap:8,
        }}>
  ...
  // useEffect para sincronizar resLocal cuando cambia grupo/fecha/ress (FUERA del if)
  useEffect(()=>{
    if(scr!=="admin")return;
    const ps=PARTIDOS.filter(p=>p.g===grupo&&p.f===fecha);
    const init={};
    ps.forEach(p=>{ init[p.id]={l:ress[p.id]?.l||"",v:ress[p.id]?.v||""}; });
    setResLocal(init);
    setGuardadoOk({});
  },[scr,grupo,fecha]);
  ...
  if(scr==="admin"){
    ...
    async function guardarPartido(pid){
      ...
      setRess(prev=>({...prev,[pid]:{l:r.l,v:r.v}}));
      setGuardando(prev=>({...prev,[pid]:false}));
      setGuardadoOk(prev=>({...prev,[pid]:true}));
      setTimeout(()=>setGuardadoOk(prev=>({...prev,[pid]:false})),2000);
    }
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:5,flexShrink:0}}>
                      <input style={S.inpGol(false)} type="number" min="0" max="20"
                        ...
                        onChange={e=>setResLocal(prev=>({...prev,[p.id]:{...rl,l:e.target.value}}))}
                        onKeyDown={e=>{if(e.key==="Enter")guardarPartido(p.id);}}
                      />
                      <span style={{color:MUTED,fontWeight:700}}>–</span>
                      <input style={S.inpGol(false)} type="number" min="0" max="20"
                        value={rl.v} placeholder="-"
                        onChange={e=>setResLocal(prev=>({...prev,[p.id]:{...rl,v:e.target.value}}))}
                        onKeyDown={e=>{if(e.key==="Enter")guardarPartido(p.id);}}
                      />
                    </div>

...
export default function App(){
  ...
  if(scr==="tabla"){
    ...
    const [piedras,setPiedras]=useState([]); // [{id,x,y,tx,ty}]
    ...
    function tirarPiedra(e){
      ...
      const ox=touch.clientX||e.clientX;
      const oy=touch.clientY||e.clientY;
      const id=Date.now();
      setPiedras(prev=>[...prev,{id,x:ox,y:oy,tx:cx,ty:cy}]);
      setTimeout(()=>setPiedras(prev=>prev.filter(p=>p.id!==id)),600);
      setTambaleo(true);
    ...
    if(juegoActivo&&lider) return(
      <div style={{
        position:"fixed",top:0,left:0,right:0,bottom:0,
        background:"radial-gradient(ellipse at center,#1a0a00,#07070d)",
      ...
      >
        <style>{`
          @keyframes tambaleo{0%{transform:rotate(0deg)}25%{transform:rotate(-12deg)}
            50%{transform:rotate(10deg)}75%{transform:rotate(-6deg)}100%{transform:rotate(0deg)}}
          @keyframes piedraVuela{0%{opacity:1;transform:scale(1)}100%{opacity:0;transform:scale(.3)}}
          @keyframes derribado{0%{transform:rotate(0deg) translateY(0)}
            50%{transform:rotate(45deg) translateY(20px)}
            100%{transform:rotate(90deg) translateY(60px);opacity:0}}
          @keyframes estrellitas{0%{opacity:1;transform:scale(0) rotate(0deg)}
        ...
        <div style={{position:"absolute",top:20,left:0,right:0,textAlign:"center"}}>
          ...
          <p style={{color:"#555",fontSize:11,margin:"4px 0 0"}}>
            {derribado?"":"Tocá la pantalla para tirar piedras"}
          </p>
        </div>

        {/* Puntos restantes visual */}
        <div style={{position:"absolute",top:60,left:0,right:0,textAlign:"center"}}>
          <span style={{
            fontSize:48,fontWeight:900,
            background:`linear-gradient(90deg,${GOLD},#fff)`,
            WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",
          }}>{Math.max(0,liderPts - golpes*3)} pts</span>
          <span style={{color:MUTED,fontSize:13,marginLeft:8}}>visual</span>
        </div>

        {/* Foto del líder */}
        <div style={{
          marginTop:20,
          animation: derribado?"derribado .8s ease forwards":
                     tambaleo?"tambaleo .3s ease":"none",
        }}>
          {lider.fotoUrl
            ...
            :<div style={{
                width:180,height:180,borderRadius:"50%",background:"#111",
                border:`5px solid ${GOLD}`,display:"flex",alignItems:"center",
        ...
        {/* Nombre */}
        <div style={{
          marginTop:16,fontSize:32,fontWeight:900,
          textTransform:"uppercase",letterSpacing:3,
          color:derribado?RED:TEXT,
          animation:derribado?"none":tambaleo?"tambaleo .3s ease":"none",
        }}>{lider.nombre}</div>

        {/* Contador de golpes */}
        <div style={{marginTop:12,display:"flex",gap:6}}>
          {Array.from({length:5}).map((_,i)=>(
            <span key={i} style={{fontSize:22,opacity:i<golpes?1:.2}}>🪨</span>
          ))}
        </div>
        ...
        {derribado&&(
          <div style={{
            ...
            animation:"estrellitas .5s ease",
          }}>
            <div style={{fontSize:48}}>💥⭐💥</div>
            <p style={{color:GOLD,fontWeight:900,fontSize:20,
              textTransform:"uppercase",letterSpacing:3,margin:"8px 0"}}>
              ¡Derribado!
            </p>
            <p style={{color:MUTED,fontSize:13}}>
              (los puntos reales no se modifican 😅)
            </p>
          </div>
        )}
        ...
        {piedras.map(p=>(
          <div key={p.id} style={{
            ...
            animation:"piedraVuela .6s ease forwards",
        ...
        <div style={{position:"absolute",bottom:24,display:"flex",gap:10}}>
          <button style={S.btn(derribado?GREEN:"#1a1a2a",derribado?"#000":"#888","1px solid #333",
            {fontSize:13,padding:"10px 20px"})}
            onClick={e=>{e.stopPropagation();setJuegoActivo(false);setGolpes(0);setDerribado(false);}}>
            {derribado?"🏆 Ver tabla":"← Salir"}
          </button>
          {golpes>0&&!derribado&&(
            <button style={S.btn("#1a0808",RED,"1px solid #601010",{fontSize:13,padding:"10px 20px"})}
              onClick={e=>{e.stopPropagation();setGolpes(0);}}>
              Resetear
            </button>
          )}
        </div>
  ...
  if(scr==="admin"){
    ...
    // Estado local para edición sin guardar automáticamente
    const [resLocal,setResLocal]=useState({});
    const [guardando,setGuardando]=useState({});
    const [guardadoOk,setGuardadoOk]=useState({});

    // Inicializar resLocal con los datos del servidor cuando cambia grupo/fecha
    useEffect(()=>{
      const init={};
      partidos.forEach(p=>{
      ...
      setResLocal(init);
      setGuardadoOk({});
    },[grupo,fecha,JSON.stringify(partidos.map(p=>p.id))]);

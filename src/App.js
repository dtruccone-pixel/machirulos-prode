...
export default function App(){
  ...
  if(scr==="tabla"){
    ...
    // Minijuego piedrazo — animación CSS parabólica
    function tirarPiedra(e){
      if(derribado)return;
      const rect=e.currentTarget.getBoundingClientRect();
      const ox=rect.left+rect.width*0.28;
      const oy=rect.top+rect.height*0.35;
      const tx=rect.left+rect.width*0.73;
      const ty=rect.top+rect.height*0.32;
      const id=Date.now()+Math.random();
      setPiedras(prev=>[...prev,{id,ox,oy,tx,ty}]);
      setTimeout(()=>setPiedras(prev=>prev.filter(p=>p.id!==id)),700);
      setTambaleo(true);
      setTimeout(()=>setTambaleo(false),400);
      const ng=golpes+1;
      setGolpes(ng);
    ...
    if(juegoActivo&&lider) return(
      ...
      >
        ...
        <div style={{
          ...
          {!derribado&&(
            <div style={{
              ...
              {/* SVG barrabrava realista — postura de lanzamiento */}
              <svg width="140" height="200" viewBox="0 0 140 200"
                style={{overflow:"visible",marginTop:-8}}>
                <defs>
                  {/* Camiseta Argentina — rayas celeste y blanca */}
                  <pattern id="argStripes" x="0" y="0" width="10" height="1" patternUnits="userSpaceOnUse">
                    <rect x="0" y="0" width="5" height="1" fill="#74b9ff"/>
                    <rect x="5" y="0" width="5" height="1" fill="#ffffff"/>
                  </pattern>
                  {/* Sombra suave */}
                  ...
                  <radialGradient id="skinGrad" cx="50%" cy="40%" r="60%">
                    ...
                    <stop offset="100%" stopColor="#c8724a"/>
                  </radialGradient>
                </defs>

                {/* ── PIERNA DERECHA (adelante, doblada) ── */}
                {/* Muslo derecho — hacia adelante */}
                <path d="M 68 108 Q 60 130 52 150" stroke="#3d5a8a" strokeWidth="22" strokeLinecap="round" fill="none"/>
                {/* Rodilla derecha */}
                <circle cx="52" cy="150" r="11" fill="#4a6fa5"/>
                {/* Canilla derecha */}
                <path d="M 52 150 Q 48 168 44 182" stroke="#3d5a8a" strokeWidth="18" strokeLinecap="round" fill="none"/>
                {/* Zapatilla derecha */}
                <ellipse cx="40" cy="186" rx="18" ry="8" fill="#1a1a1a" transform="rotate(-15,40,186)"/>
                <ellipse cx="32" cy="184" rx="9" ry="6" fill="#333" transform="rotate(-15,32,184)"/>
                {/* Suela */}
                <path d="M 24 188 Q 38 194 56 190" stroke="#555" strokeWidth="3" fill="none"/>

                {/* ── PIERNA IZQUIERDA (atrás, extendida) ── */}
                {/* Muslo izquierdo — atrás y extendido */}
                <path d="M 72 108 Q 86 128 98 148" stroke="#3d5a8a" strokeWidth="22" strokeLinecap="round" fill="none"/>
                {/* Canilla izquierda */}
                <path d="M 98 148 Q 104 165 108 182" stroke="#3d5a8a" strokeWidth="18" strokeLinecap="round" fill="none"/>
                {/* Zapatilla izquierda */}
                <ellipse cx="112" cy="186" rx="18" ry="8" fill="#1a1a1a" transform="rotate(10,112,186)"/>
                <ellipse cx="120" cy="184" rx="9" ry="6" fill="#333" transform="rotate(10,120,184)"/>
                <path d="M 96 188 Q 112 194 128 188" stroke="#555" strokeWidth="3" fill="none"/>

                {/* ── JEAN — cintura ── */}
                <path d="M 52 106 Q 70 115 88 106" stroke="#2c3e6b" strokeWidth="12" strokeLinecap="round" fill="none"/>

                {/* ── TORSO — camiseta Argentina ── */}
                {/* Cuerpo principal inclinado levemente */}
                <path d="M 55 55 Q 50 80 52 108 Q 70 116 88 108 Q 90 80 85 55 Z"
                  fill="url(#argStripes)" filter="url(#shadow)"/>
                {/* Cuello camiseta */}
                <path d="M 62 55 Q 70 60 78 55 Q 76 48 70 46 Q 64 48 62 55 Z" fill="#ffffff"/>
                {/* Escudo AFA pequeño */}
                <rect x="60" y="62" width="10" height="12" rx="2" fill="#1a3a7a"/>
                <text x="65" y="72" fontSize="7" textAnchor="middle" fill="#f0c040">⭐</text>

                {/* ── BRAZO IZQUIERDO (adelante, equilibrio) ── */}
                {/* Hombro */}
                <circle cx="52" cy="60" r="10" fill="#74b9ff"/>
                {/* Brazo superior */}
                <path d="M 52 60 Q 35 70 22 78" stroke="#f0a882" strokeWidth="14" strokeLinecap="round" fill="none"/>
                {/* Antebrazo */}
                <path d="M 22 78 Q 12 85 6 90" stroke="#f0a882" strokeWidth="12" strokeLinecap="round" fill="none"/>
                {/* Mano abierta equilibrio */}
                <circle cx="4" cy="92" r="8" fill="url(#skinGrad)"/>
                <path d="M -2 88 Q -4 80 -2 76" stroke="#c8724a" strokeWidth="4" strokeLinecap="round" fill="none"/>
                <path d="M 2 87 Q 0 79 2 74" stroke="#c8724a" strokeWidth="4" strokeLinecap="round" fill="none"/>
                <path d="M 7 88 Q 6 80 8 76" stroke="#c8724a" strokeWidth="4" strokeLinecap="round" fill="none"/>

                {/* ── BRAZO DERECHO (atrás arriba, con piedra) ── */}
                {/* Hombro */}
                <circle cx="88" cy="60" r="10" fill="#74b9ff"/>
                {/* Brazo superior — va hacia atrás y arriba */}
                <g style={{
                  transformOrigin:"88px 60px",
                  animation:`brazoAtras ${tambaleo?.25:.8}s ease-in-out ${tambaleo?"forwards":"infinite"}`,
                }}>
                  <path d="M 88 60 Q 110 40 124 22" stroke="#f0a882" strokeWidth="14" strokeLinecap="round" fill="none"/>
                  {/* Antebrazo */}
                  <path d="M 124 22 Q 132 14 136 8" stroke="#f0a882" strokeWidth="12" strokeLinecap="round" fill="none"/>
                  {/* Puño cerrado con PIEDRA */}
                  <circle cx="138" cy="6" r="9" fill="url(#skinGrad)"/>
                  {/* Nudillos */}
                  <circle cx="134" cy="2" r="3" fill="#c8724a"/>
                  <circle cx="139" cy="0" r="3" fill="#c8724a"/>
                  <circle cx="144" cy="2" r="3" fill="#c8724a"/>
                  {/* PIEDRA en la mano */}
                  <ellipse cx="144" cy="-4" rx="13" ry="11" fill="#7f8c8d" transform="rotate(-20,144,-4)"/>
                  <ellipse cx="141" cy="-7" rx="6" ry="4" fill="#95a5a6" opacity=".7" transform="rotate(-20,141,-7)"/>
                  <ellipse cx="148" cy="-1" rx="4" ry="3" fill="#636e72" opacity=".5"/>
                  <path d="M 136 -14 Q 144 -18 152 -12" stroke="#555" strokeWidth="1.5" fill="none" opacity=".6"/>
                </g>

                {/* ── CUELLO ── */}
                <rect x="64" y="40" width="12" height="16" rx="5" fill="url(#skinGrad)"/>

                {/* ── CABEZA — con foto del jugador ── */}
                {/* Fondo de cabeza */}
                <circle cx="70" cy="30" r="26" fill="#111" filter="url(#shadow)"/>
                {/* La foto se inserta via foreignObject */}
                <clipPath id="headClip">
                  <circle cx="70" cy="30" r="24"/>
                </clipPath>
                {jugador?.fotoUrl
                  ?<image href={jugador.fotoUrl} x="46" y="6" width="48" height="48"
                      clipPath="url(#headClip)" preserveAspectRatio="xMidYMid slice"/>
                  :<text x="70" y="38" fontSize="28" textAnchor="middle">😤</text>
                }
                {/* Aro de la foto */}
                <circle cx="70" cy="30" r="24" fill="none" stroke="#e63946" strokeWidth="3"/>

                {/* Gorra/gorrito opcional */}
                <path d="M 46 20 Q 70 5 94 20 Q 90 12 70 8 Q 50 12 46 20 Z" fill="#1a3a7a" opacity=".8"/>
        ...
        {/* Piedras volando — CSS parabólico */}
        {piedras.map(p=>(
          <div key={p.id}>
            <style>{`
              @keyframes vueloX_${p.id.toString().replace(".","")}{
                from{left:${p.ox}px}
                to{left:${p.tx}px}
              }
              @keyframes vueloY_${p.id.toString().replace(".","")}{
                0%{top:${p.oy}px;transform:translate(-50%,-50%) scale(.8) rotate(0deg)}
                40%{top:${Math.min(p.oy,p.ty)-90}px;transform:translate(-50%,-50%) scale(1) rotate(288deg)}
                100%{top:${p.ty}px;transform:translate(-50%,-50%) scale(1.1) rotate(720deg)}
              }
            `}</style>
            <div style={{
              position:"fixed",
              ...
              zIndex:50,
              fontSize:28,
              animationName:`vueloX_${p.id.toString().replace(".","")},vueloY_${p.id.toString().replace(".","")}`,
              animationDuration:"0.6s,0.6s",
              animationTimingFunction:"linear,ease-in",
              animationFillMode:"forwards,forwards",
              filter:"drop-shadow(3px 5px 4px rgba(0,0,0,.6))",
            }}>🪨</div>
          </div>
        ))}

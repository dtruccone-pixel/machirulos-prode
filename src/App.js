...
function IntroScreen({ jugadores, onDone }) {
  const [fase, setFase]       = useState("logo");    // logo | desfile | final
  const [actual, setActual]   = useState(0);
  const [visible, setVisible] = useState(false);
  const timerRef = useRef(null);

  // Fase 1: mostrar logo 1.8s → arrancar desfile
  useEffect(() => {
    const t = setTimeout(() => {
  ...
  useEffect(() => {
    ...
    if (actual >= jugadores.length) {
      // Terminó el desfile → pantalla final
      timerRef.current = setTimeout(() => setFase("final"), 400);
      return;
    }
    setVisible(true);
    timerRef.current = setTimeout(() => {
      setVisible(false);
      setTimeout(() => setActual(a => a + 1), 350);
    }, 1200);
    return () => clearTimeout(timerRef.current);
  }, [fase, actual, jugadores.length]);

  // Fase 3: pantalla final → ir al inicio
  useEffect(() => {
  ...
  if (fase === "logo") return (
    <div style={{
      ...
      <style>{`
        ...
        @keyframes pulseGold { 0%,100%{text-shadow:0 0 30px #f0c04066} 50%{text-shadow:0 0 80px #f0c040cc,0 0 120px #f0c04044} }
        ...
        @keyframes confettiDrop { from{opacity:1;transform:translateY(-20px) rotate(0deg)} to{opacity:0;transform:translateY(100vh) rotate(720deg)} }
        @keyframes shimmer { 0%,100%{opacity:.7} 50%{opacity:1} }
      `}</style>
      <div style={{ textAlign: "center" }}>
        <div style={{ fontSize: 90, animation: "pulseGold 2s infinite", marginBottom: 16 }}>🏆</div>
        <div style={{
          fontSize: "clamp(48px,12vw,90px)", fontWeight: 900, letterSpacing: 6,
        ...
        <div style={{
          fontSize: "clamp(14px,3vw,20px)", color: "#888", letterSpacing: 4,
          textTransform: "uppercase", marginTop: 12,
          animation: "slideUp .8s ease .6s both",
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          color: "#333", fontSize: 12, letterSpacing: 3, textTransform: "uppercase",
        }}>
        ...
        <div style={{
          ...
          animation: visible ? "floatIn .35s ease both" : "floatOut .35s ease both",
        }}>
          {/* Foto o avatar */}
          {j.fotoUrl
            ? <img src={j.fotoUrl} alt={j.nombre} style={{
                ...
                boxShadow: `0 0 60px ${GOLD}55, 0 0 120px ${GOLD}22`,
              }}/>
            : <div style={{
                width: 160, height: 160, borderRadius: "50%",
                background: "#111",
          ...
          {/* Nombre */}
          <div style={{
            fontSize: "clamp(32px,8vw,64px)", fontWeight: 900,
          ...
          <div style={{ fontSize: 20, letterSpacing: 8, opacity: .6 }}>⭐⭐⭐</div>
        </div>
      </div>
    );
  }
  ...
  // ── FINAL: todos juntos ──
  if (fase === "final") {
    const cols = jugadores.length <= 4 ? 2 :
                 jugadores.length <= 6 ? 3 :
                 jugadores.length <= 9 ? 3 : 4;
    return (
      <div style={{
        ...base,
        background: "radial-gradient(ellipse at center, #1a0f00 0%, #07070d 70%)",
        ...
        <div style={{
          fontSize: "clamp(20px,5vw,36px)", fontWeight: 900,
        ...
        <div style={{
          ...
          animation: "zoomIn .6s ease .1s both",
        }}>
          {jugadores.map((j, i) => (
            <div key={j.nombre} style={{
              display: "flex", flexDirection: "column",
              alignItems: "center", gap: 6,
              animation: `floatIn .4s ease ${i * .08}s both`,
            }}>
              {j.fotoUrl
                ? <img src={j.fotoUrl} alt={j.nombre} style={{
                ...
                : <div style={{
                    width: 56, height: 56, borderRadius: "50%",
                    background: "#111", border: `2px solid ${GOLD}`,
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: 26,
                  }}>👤</div>
              }

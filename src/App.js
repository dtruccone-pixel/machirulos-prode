...
import { PARTIDOS, GRUPOS_KEYS, FLAG, calcPts, GRUPOS, FASES_KO, getPartidosKO } from "./datos";
...
const LIMITE_LABELS={1:"Cierre: 11 Jun 15:59",2:"Cierre: 13 Jun 22:59",3:"Cierre: 19 Jun 15:59"};

function estaBloquadaFecha(fecha,envios,nombre,desbloqueos){
  const key=nombre+":fecha"+fecha;
...
function calcTablaGrupos(jds,prons,ress){
  return jds.map(j=>{
...
function fechaGruposCompleta(ress,f){
  return PARTIDOS.filter(p=>p.f===f).every(p=>{const r=ress[p.id];return r&&r.l!==""&&r.v!=="";});
}
...
export default function App(){
  ...
  const [envios,setEnvios]     = useState({});      // bloqueos fase grupos
  ...
  const [eqLocal,setEqLocal]   = useState({});  // equipos KO editados localmente

  // useEffect admin sync
  useEffect(()=>{
    if(scr!=="admin")return;
    const init={};
    PARTIDOS.filter(p=>p.g===grupo&&p.f===fecha).forEach(p=>{
  ...
  if(scr==="inicio")return(
    <div style={S.root}>
      ...
      <div style={{...S.wrap,display:"flex",flexDirection:"column",gap:10,marginTop:10}}>
        ...
        {/* Sección Grupos */}
        <div style={{background:"linear-gradient(135deg,#0a1a0a,#0d0d1a)",border:`2px solid ${GREEN}`,borderRadius:14,padding:"18px 16px"}}>
          ...
          <button style={S.btn(`linear-gradient(90deg,${GREEN},#1a6a30)`,"#fff","none",{width:"100%",padding:"13px"})}
            onClick={()=>setScr("elegir")}>⚽ Pronosticos de grupos</button>
        </div>
  ...
  if(scr==="elegir")return(
    <div style={S.root}>
      ...
      <div style={S.wrap}>
        <div style={S.card}>
          {jds.length===0
            ?<p style={{color:MUTED,textAlign:"center",padding:"20px 0"}}>El admin tiene que agregar los jugadores primero.</p>
            :<div style={{display:"flex",flexDirection:"column",gap:8}}>
              {jds.map(j=>{
                ...
                return(
                  <button key={j.nombre} style={{...S.btn("#0d0d18","#fff","1px solid "+BDR),textAlign:"left",padding:"12px 16px",display:"flex",alignItems:"center",gap:12}}
      ...
      <div style={S.wrap}>
        <div style={S.card}>
          <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:20}}>
            ...
            <div>
              <p style={{...S.sTitle,margin:0}}>Verificacion</p>
              <p style={{fontSize:22,fontWeight:800,margin:"4px 0 0"}}>{jLogin?.nombre}</p>
            </div>
          </div>
          <p style={{color:MUTED,fontSize:13,margin:"0 0 12px"}}>Ingresa tu numero de DNI</p>
  ...
  // ── JUGADOR — FASE DE GRUPOS ──
  if(scr==="jugador"){
    const mp=prons[jugador.nombre]||{};
    const partidos=PARTIDOS.filter(p=>p.g===grupo&&p.f===fecha);
    const bloqueada=estaBloquadaFecha(fecha,envios,jugador.nombre,desbloqueos);
    const resto=tiempoRestante(fecha);

    if(confirmEnvio!==null){
      const f=confirmEnvio;
      const comp=PARTIDOS.filter(p=>p.f===f).filter(p=>{const pr=mp[p.id];return pr&&pr.l!==""&&pr.v!=="";}).length;
      ...
      return(
        <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,zIndex:500,
          ...
          <div style={{...S.card,maxWidth:400,width:"100%",textAlign:"center"}}>
            ...
            <p style={{color:TEXT,fontSize:15,margin:"0 0 6px"}}>{comp}/{total} partidos cargados.</p>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"center"}}>
              <button style={S.btn(RED,"#fff","none",{padding:"12px 24px"})} disabled={enviando}
                onClick={async()=>{
                  setEnviando(true);
                  const r=await API.enviarFase(jugador.nombre,"fecha:"+f);
                  if(r?.ok)setEnvios(prev=>({...prev,[jugador.nombre+":fecha"+f]:true}));
                  setEnviando(false);setConfirmEnvio(null);
                }}>{enviando?"Enviando…":"Si, enviar"}</button>
              <button style={S.btn("#1a1a2a","#ccc","1px solid #333",{padding:"12px 24px"})}
                onClick={()=>setConfirmEnvio(null)}>Cancelar</button>
            </div>
          </div>
        </div>
      );
    ...
    async function setPron(pid,campo,val){
      ...
      await API.setPronostico(jugador.nombre,pid,"grupos",pr.l||"",pr.v||"");
    }

    const otrosJds=jds.filter(j=>j.nombre!==jugador.nombre);
      ...
      <div style={S.root}>
        ...
        <div style={S.wrap}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              <button style={S.btn("transparent","#e67e22","1px solid #e67e22")}
                onClick={()=>setScr("inicio")}>⚔ Fases KO</button>
          ...
          <div style={{display:"flex",gap:8,marginBottom:10}}>
            {FECHAS.map(f=>{
              const bloq=estaBloquadaFecha(f,envios,jugador.nombre,desbloqueos);
          ...
          <div style={{padding:"10px 14px",borderRadius:10,marginBottom:12,
            ...
            <div>
              {bloqueada
                ?<span style={{color:GREEN,fontWeight:700,fontSize:14}}>🔒 Fecha {fecha} enviada</span>
                :<span style={{color:GOLD,fontWeight:700,fontSize:14}}>⏰ {LIMITE_LABELS[fecha]}
                  {resto&&<span style={{marginLeft:8,color:GREEN}}> · {resto}</span>}
                </span>
              }
            </div>
            {!bloqueada&&(
              <button style={S.btn(`linear-gradient(90deg,${GREEN},#1a6a30)`,"#fff","none",{fontSize:13,padding:"9px 18px"})}
                onClick={()=>setConfirmEnvio(fecha)}>📤 Enviar Fecha {fecha}</button>
            )}
          </div>

          {/* Ver todos toggle */}
          <div style={{marginBottom:10,display:"flex",gap:8}}>
            <button style={S.btn(verProns?"transparent":"#1a1a2c",verProns?GOLD:TEXT,verProns?`1px solid ${GOLD}`:`1px solid ${BDR}`)}
              ...
              return(
                <div key={p.id} style={{...S.card,marginBottom:10}}>
                  <div style={{fontSize:10,color:MUTED,marginBottom:8}}>
                    {p.fecha} · Grupo {p.g} · {FLAG[p.local]||""} {p.local} vs {p.vis} {FLAG[p.vis]||""}
                  ...
                  {[jugador,...otrosJds].map(j=>{
                    ...
                    return(
                      <div key={j.nombre} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",borderRadius:8,marginBottom:5,
                        ...
                        <span style={{fontSize:15,fontWeight:700,color:tiene?TEXT:MUTED}}>
                          {tiene?`${pr.l} - ${pr.v}`:"sin cargar"}
                        </span>
                        {pts!==null&&<span style={{fontSize:14,fontWeight:900,color:ptC}}>{pts===3?"🎯":pts===2?"✅":"❌"}</span>}
                      </div>
                    );
                  })}
                </div>
            ...
            partidos.map(p=>{
              ...
              return(
                <div key={p.id} style={{background:bg,borderRadius:12,marginBottom:10,padding:14,border:`2px solid ${bc}`}}>
                  <div style={{fontSize:10,color:MUTED,marginBottom:10}}>📅 {p.fecha} · {p.hora} · {p.sede} · Grupo {p.g}</div>
                  <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
                      <input style={S.inpGol(bloqueada||hRes)} type="text" inputMode="numeric" maxLength={2}
                        ...
                        onChange={e=>setPron(p.id,"l",e.target.value.replace(/[^0-9]/g,""))}/>
                      <span style={{color:MUTED,fontWeight:700,fontSize:20}}>-</span>
                      <input style={S.inpGol(bloqueada||hRes)} type="text" inputMode="numeric" maxLength={2}
                        value={pron.v} placeholder="-" disabled={bloqueada||hRes}
                        onChange={e=>setPron(p.id,"v",e.target.value.replace(/[^0-9]/g,""))}/>
                    </div>
                  ...
                  {hRes&&(
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:12,paddingTop:12,borderTop:"1px solid #1a1a2c"}}>
                      <div>
                        <div style={{color:"#3a3a6a",fontSize:11}}>Resultado real</div>
    ...
    return(
      <div style={S.root}>
        ...
        <div style={S.wrap}>
          ...
          {/* Login prompt */}
          {!jugador&&(
            <div style={{...S.card,textAlign:"center",padding:20,marginBottom:10}}>
              <p style={{color:MUTED,margin:"0 0 12px"}}>Inicia sesion para cargar pronosticos</p>
              <button style={S.btn(`linear-gradient(90deg,${GOLD},#b88000)`)} onClick={()=>setScr("elegir")}>⚽ Ingresar</button>
            </div>
          )}
  ...
  if(scr==="tabla"){
    ...
    return(
      <div style={S.root}>
        <Header sub={`${jugados}/72 partidos grupos jugados`} sync={sync}/>
        <div style={S.wrap}>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
            ...
            <button style={S.tab(tabFase==="grupos")} onClick={()=>setTabFase("grupos")}>Grupos</button>
            {FASES_KO.map(f=>(
          ...
          {tabFase==="grupos"&&(()=>{
            const t=calcTablaGrupos(jds,prons,ress);
            const mx=t[0]?.pts??0;
            return(
              <div style={S.card}>
                <p style={S.sTitle}>⚽ Tabla Fase de Grupos</p>
                {t.length===0?<p style={{color:MUTED}}>Sin puntos</p>
                  :t.map((item,i)=><TablaFila key={item.jugador.nombre} item={item} pos={i} maxPts={mx}/>)}
              </div>
            );
          })()}
  ...
  // ── ADMIN ──
  if(scr==="admin"){
    const partGrupos=PARTIDOS.filter(p=>p.g===grupo&&p.f===fecha);
      ...
      <div style={S.root}>
        ...
        <div style={S.wrap}>
          <div style={{display:"flex",gap:7,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>
            ...
            <button style={S.btn("#1a1a2a","#888","1px solid #333")} onClick={cargarTodo}>🔄</button>
            <span style={{color:MUTED,fontSize:11}}>{cargados}/72 grupos</span>
          </div>
          ...
          {/* RESULTADOS GRUPOS */}
          <div style={S.card}>
            <p style={S.sTitle}>⚽ Resultados Fase de Grupos</p>
            <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:10}}>
              {GRUPOS_KEYS.map(g=>{
                const done=PARTIDOS.filter(p=>p.g===g).every(p=>{const r=ress[p.id];return r&&r.l!==""&&r.v!=="";});
                return<button key={g} style={{...S.tab(grupo===g),...(done&&grupo!==g?{borderColor:"#2a4a2a",color:"#3aaa3a"}:{})}}
            ...
            <div style={{display:"flex",justifyContent:"flex-end",marginBottom:8}}>
              <button style={S.btn(GREEN,"#000","none",{fontSize:12,padding:"8px 16px"})}
                onClick={async()=>{for(const p of partGrupos){const r=resLocal[p.id]||{l:"",v:""};if(String(r.l).trim()&&String(r.v).trim())await guardarResultado(p.id);}}}>
            ...
            {partGrupos.map(p=>{
              ...
              const yaG=ress[p.id]?.l!==""&&ress[p.id]?.v!=="";
              return(
                <div key={p.id} style={{padding:"10px 0",borderBottom:`1px solid ${BDR}`}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                    <span style={{fontSize:11,color:MUTED,width:50,flexShrink:0}}>{p.fecha}</span>
                    <div style={{flex:"1 1 90px",display:"flex",alignItems:"center",gap:4}}>
                      <span style={{fontSize:16}}>{FLAG[p.local]||""}</span><span style={{fontSize:13}}>{p.local}</span>
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:4,flexShrink:0}}>
                      <input style={{...S.inpGol(false),fontSize:16}} type="text" inputMode="numeric" maxLength={2} value={rl.l} placeholder="-"
                      ...
                      <input style={{...S.inpGol(false),fontSize:16}} type="text" inputMode="numeric" maxLength={2} value={rl.v} placeholder="-"
                        onChange={e=>setResLocal(prev=>({...prev,[p.id]:{...(prev[p.id]||{l:"",v:""}),v:e.target.value.replace(/[^0-9]/g,"")}}))
                        } onKeyDown={e=>{if(e.key==="Enter")guardarResultado(p.id);}}/>
                    </div>
                    <div style={{flex:"1 1 90px",display:"flex",alignItems:"center",justifyContent:"flex-end",gap:4}}>
                      <span style={{fontSize:13}}>{p.vis}</span><span style={{fontSize:16}}>{FLAG[p.vis]||""}</span>
                    </div>
                    <button disabled={!listo||cP} onClick={()=>guardarResultado(p.id)}
                      style={{padding:"6px 10px",borderRadius:7,cursor:listo&&!cP?"pointer":"default",fontWeight:800,fontSize:11,fontFamily:"inherit",flexShrink:0,
                        background:gok?"#0f2a0f":listo&&!sinC?GREEN:"#1a1a2a",
                        border:`1px solid ${gok?GREEN:listo&&!sinC?GREEN:"#333"}`,
          ...
          <div style={S.card}>
            ...
            {jds.map(j=>(
              <div key={j.nombre} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",
                ...
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  {FECHAS.map(f=>{
                    const key=j.nombre+":fecha"+f;
                    const desbloq=desbloqueos[key];
                    const bloq=estaBloquadaFecha(f,envios,j.nombre,desbloqueos);
                    return(
                      <button key={f} onClick={async()=>{
                        if(desbloq){await API.bloquearJugador(j.nombre,"fecha:"+f);setDesbloq(p=>{const n={...p};delete n[key];return n;});}
                        else{await API.desbloquearJugador(j.nombre,"fecha:"+f);setDesbloq(p=>({...p,[key]:true}));}

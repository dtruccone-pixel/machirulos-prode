...
// Bloqueo por envío + fecha límite + desbloqueo admin
// ══════════════════════════════════════════════════════════════════
...
function estaBloquada(fecha, envios, jugador, desbloqueos) {
  const key = jugador + ":f" + fecha;
  // Si el admin habilitó al jugador explícitamente → NO bloqueado
  if (desbloqueos && desbloqueos[key] === true) return false;
  // Bloqueo por fecha límite
  if (new Date() >= FECHAS_LIMITE[fecha]) return true;
  // Bloqueo por envío voluntario
...
function getSpreadsheet() {
  ...
  ["jugadores","resultados","pronosticos","envios","desbloqueos","config"].forEach(n => getOrCreateSheet(ss, n));
  return ss;
}
...
function doPost(e) {
  try {
    ...
    const desbloqueos= getDesbloqueos(ss);
    ...
    switch(body.action) {
      ...
      case "setPronostico":
        if (estaBloquada(body.fecha, envios, body.jugador, desbloqueos)) {
      ...
      case "desbloquearJugador":
        // Admin habilita a un jugador para modificar una fecha
        setDesbloqueo(ss, body.jugador, body.fecha, true);
        return jsonResp({ ok: true });

      case "bloquearJugador":
        // Admin vuelve a bloquear
        setDesbloqueo(ss, body.jugador, body.fecha, false);
        return jsonResp({ ok: true });
...
// ── DESBLOQUEOS (admin) ───────────────────────────────────────────
// Estructura: jugador | fecha | habilitado (true/false)

function getDesbloqueos(ss) {
  const sh   = getOrCreateSheet(ss, "desbloqueos");
  const data = sh.getDataRange().getValues();
  const res  = {};
  for (let i = 1; i < data.length; i++) {
    const [jugador, fecha, habilitado] = data[i];
    if (jugador && fecha && habilitado === true) {
      res[jugador + ":f" + fecha] = true;
    }
  }
  return res;
}

function setDesbloqueo(ss, jugador, fecha, habilitado) {
  const sh   = getOrCreateSheet(ss, "desbloqueos");
  const data = sh.getDataRange().getValues();
  if (!data.length || data[0][0] !== "jugador") {
    sh.getRange(1,1,1,3).setValues([["jugador","fecha","habilitado"]]);
  }
  const rows = sh.getDataRange().getValues();
  for (let i = 1; i < rows.length; i++) {
    if (rows[i][0]===jugador && String(rows[i][1])===String(fecha)) {
      sh.getRange(i+1,3).setValue(habilitado); return;
    }
  }
  sh.appendRow([jugador, fecha, habilitado]);
}
